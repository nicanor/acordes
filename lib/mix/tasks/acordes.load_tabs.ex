defmodule Mix.Tasks.Acordes.LoadTabs do
  ...
  defp load_artist(id, slug) do
    ...
    body["tabs"]
    |> Task.async_stream(fn %{"title" => tab_title, "url" => tab_url} -> load_tab(id, tab_title, tab_url) end, [timeout: 20000])
    |> Stream.run()
  end
  ...
  defp load_tab(id, title, url) do
    ...
    case Acordes.Hub.get_tab(id, slug) do
      nil ->
        ...
        Acordes.Hub.create_tab(%{title: title, slug: slug, content: body["content"], artist_id: id})
      _ -> nil
    end
  end
end
